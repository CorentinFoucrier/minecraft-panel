...
$('#editRolePermissions').on({
    "shown.bs.modal": e => { // Trigger when modal is shown
        $.post('/settings/get_role_permission', {
            token: token,
            role: g_role
        }, data => {
            if (data) {
    ...
    "hidden.bs.modal": e => { // Trigger when modal is hidden
...
$('input[type=checkbox].permission').on("click", e => {
    ...
    $.post('/settings/edit_role_permission', {
        token: token,
        checked: checked,
...
const addNewUser = () => {
    ...
    $.post('/settings/add_new_user', {
        ...
        if (data) {
            switch (data.state) {
                case "success":
                    toastr.success(data.success.message, data.success.title);
...
const deleteUser = (username) => {
    $.post('/settings/delete_user', {
        ...
        if (data) {
            switch (data.state) {
                case "deleted":
                    toastr.success(data.deleted.message, data.deleted.title);
                    deleteRow(username);
                    break;
                case "notExist":
                    toastr.error(data.notExist.message, data.notExist.title);
                    deleteRow(username);
                    break;
                case "error":
                    toastr.error(data.error.message, data.error.title);
...
const saveRolesOrder = () => {
    ...
    $.post('/settings/save_roles_order', {
        token: token,
        ...
        if (data) {
            switch (data.state) {
                case "success":
                    ...
                    $('#addNewUser_role, #editUserRole_role').append(`<option value="${data.rank}">${role.val()}</option>`);
                    role.val('');
                    toastr.success(data.success.message, data.success.title);
                    break;
            }
            ...
            switch (data.state) {
                case "success":
                    $(`
                        ...
                        #editUserRole_role option:contains("${role}")`
                    ).remove();
                    deleteRow(role);
                    toastr.success(data.success.message, data.success.title);
                    break;
